...
| `[html]...[/html]` | 自訂 HTML 區塊（CSS 隔離） | 見下方 |
| `[qa-session title="..."]...[/qa-session]` | 全版 QA 問答區塊 | 見下方 |
| `[lab-session title="..." duration="..."]...[/lab-session]` | 全版實作時間區塊 | 見下方 |
| `[survey title="..." url="..." hint="..." btn="..."]...[/survey]` | 全版滿意度問卷區塊 | 見下方 |
| `---` | 章節分隔線 | 放在 `#` 章節之間 |
...
- 每個項目以 `- label:` 開始，後續欄位縮排兩格
- 必填欄位：`date`、`time`
- 選填欄位：`label`、`marker`、`des`
- `date` 可包含星期，例如 `5/30（六）`、`5/30 (六)`、`5/ 30 (六)`
- `des` 用來描述該堂課的主題或重點
...
```
- `title` 為選填，預設 `🛠️ 實作時間`
- `duration` 為選填，顯示時間膠囊（如 `15 分鐘`）
- `hint` 為選填，預設 `有問題歡迎提出，你的問題可能是大家的問題`
- `- ` 開頭的行作為任務項目，顯示帶編號的清單
- 整體視覺風格與 `[qa-session]` 類似，但使用 `--accent2` 色系加以區分
...
```
- `title` 為選填，預設 `Q&A 時間`
- 每個問答以 `- Q:` 開頭，`  A:` 兩格縮排接在下一行
- 無 items 時呈現全版「等待提問」視覺（`min-height: 100vh` / `100dvh`）
- 有 items 時中央下方展開 accordion 列表，點擊展開/收合答案
- 完整支援深色/淺色模式與主題色切換
...
- `btn` 為選填，按鈕文字預設 `填寫問卷`
- 整個區塊包成獨立全螢幕頁面，適合放在課程最後

**Raw HTML（自訂 HTML 區塊）：**
```markdown
[html]
<div style="display:flex;gap:12px">
  <div style="flex:1;padding:16px;border:1px solid #ccc">面板 A</div>
  <div style="flex:1;padding:16px;border:1px solid #ccc">面板 B</div>
</div>
[/html]
```
- 內容以 `<iframe srcdoc>` 渲染，CSS 完全隔離不影響外層
- 可包含完整 HTML 文件（含 `<!DOCTYPE html>`、`<style>` 等），也可以是 HTML 片段
- iframe 載入後自動調整高度
- 適合需要獨立樣式的互動元件、對比圖表、自訂排版
...
> `config/global.yaml` 不需要放在固定位置。Build 的搜尋順序為：
> 1. **課程目錄父層往上 4 層**找 `config/global.yaml`（適合「同層放專屬覆蓋」的情境）
> 2. **fallback：`<repo>/course/config/global.yaml`**（本專案的預設集中化位置，跨資料夾共用同一份）
> 3. 也可用 `COURSE_GLOBAL_CONFIG=/path/to/global.yaml` 環境變數明確指定

#### 首次設定：建立 Global Config

如果還沒有 `config/global.yaml`，需要先建立全域設定。可參考 [config-example.yaml](reference/config-example.yaml) 作為模板：

1. **建議路徑**：直接放在 `<repo>/course/config/global.yaml`，這是本專案 build 腳本的預設 fallback 位置；不論課程資料夾放在 `course/`、`lecture/` 或 repo 根目錄，都會自動讀到同一份。
2. 在同層建立 `config/assets/` 資料夾，放入講師頭像（檔名為 `author`，副檔名可省略，build 會自動偵測 jpg/jpeg/png/webp/gif/svg）
3. 若要為**某個課程**單獨覆蓋 global，可改在該課程的父層另建一份 `config/global.yaml`（會優先於預設 fallback）
