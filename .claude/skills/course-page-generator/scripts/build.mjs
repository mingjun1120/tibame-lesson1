  ...
  while (i < lines.length) {
    ...
    if (/^### /.test(line)) {
      ...
      while (i < lines.length) {
        ...
        if (/^```(prompt|terminal|code)/i.test(cl) || /^\[flow\]/.test(cl) || /^\[tags\]/.test(cl) || /^\[summary\]/.test(cl) || /^\[bonus/.test(cl) || /^> \*\*/.test(cl) || /^\[image-text/.test(cl) || /^\[youtube/.test(cl) || /^\[html\]/.test(cl) || /^\[qa-session/.test(cl) || /^\[lab-session/.test(cl) || /^\[survey/.test(cl)) break;
    ...
    if (/^\[time\]/.test(line.trim())) {
      ...
      while (i < lines.length && !/^\[\/time\]/.test(lines[i].trim())) {
        ...
        if (bulletField) {
          currentItem = { index: items.length + 1, label: '', marker: '', date: '', weekday: '', time: '', des: '' };
          ...
          currentItem = { ...parsed, label: '', marker: '' };
          items.push(currentItem);
        }
        i++;
      }
      i++;
      if (current) current.blocks.push({ type: 'class-timeline', items: items.filter(item => item.date || item.time || item.des) });
      continue;
    }
    ...
    if (/^- /.test(line) && !/^- \[(x|green|orange|purple|blue)\]/.test(line) && current) {
      const items = [];
      while (i < lines.length && /^- /.test(lines[i]) && !/^- \[(x|green|orange|purple|blue)\]/.test(lines[i])) {
...
${childrenHtml}
    ...
    case 'class-timeline': {
      let s = `<div class="class-timeline">\n`;
      block.items.forEach((item) => {
        const weekday = formatWeekdayLabel(item.weekday);
        const marker = item.marker || item.index;
        const label = item.label || `第 ${item.index} 堂`;
        s += `      <div class="class-timeline__item">
        <div class="class-timeline__marker">${esc(String(marker))}</div>
...
function buildContentSections(sections) {
  ...
  for (const sec of sections) {
    ...
    for (const b of sec.blocks) {
      if (b.type === 'qa-session' || b.type === 'lab-session' || b.type === 'survey') {
  ...
  <div class="reveal">
    ...
    ${sec.lead ? `<p class="lead">${inlineFormatWithBreaks(sec.lead)}</p>` : ''}
      ...
      if (seg.special) {
        ...
        const pageClass = isLab ? 'lab-page' : isSurvey ? 'survey-page' : 'qa-page';
        const suffix = isLab ? 'lab' : isSurvey ? 'survey' : 'qa';
...
// 預設集中化的 global config 位置：<repo>/course/config/global.yaml
// 由 COURSE_GLOBAL_CONFIG 環境變數覆寫；找不到再 fallback 至此。
const DEFAULT_GLOBAL_CONFIG = process.env.COURSE_GLOBAL_CONFIG
  ? resolve(process.env.COURSE_GLOBAL_CONFIG)
  : resolve(process.cwd(), 'course/config/global.yaml');

function findGlobalConfig(courseDir) {
  // 1. 先從課程目錄父層往上找 4 層 config/global.yaml（保留原行為，課程旁的設定優先）
  ...
  // 2. fallback：使用 repo 預設的 course/config/global.yaml（跨資料夾共用同一份）
  if (existsSync(DEFAULT_GLOBAL_CONFIG)) return DEFAULT_GLOBAL_CONFIG;
  return null;
}
